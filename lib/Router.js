...
Router.prototype.manage = function(req, res) {
	...
	if (assets.check(urlRequested)) {
		...
		if (!validRoute.found) {
			...
			controllerPath +=  urlRequested.length > 1 ? validRoute.originalRoute : "index";
			controllerPath += ".js";
			...
			path += "app/views/";
			path += urlRequested.length > 1 ? validRoute.originalRoute : "index";
			path += this.manageExtension();
...
Router.prototype.manageExtension = function() {
	var extension = null;
